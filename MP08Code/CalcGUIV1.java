...
public class CalcGUIV1 extends JFrame implements ActionListener {
    ...
    public JPanel getButtonPanel() {
        ...
        for (int i = 10; i < 13; i++) {
            buttons[i] = new OperatorButton(calculator, this);
            buttons[i].setText(buttonText[i]);
            buttons[i].setPreferredSize(buttonDimension);
            buttons[i].addActionListener(this);
            buttonPanel.add(buttons[i]);
        ...
        for (int i = 14; i < 15; i++) {
            buttons[i] = new EqualButton(calculator, this);
            buttons[i].setText(buttonText[i]);
            buttons[i].setPreferredSize(buttonDimension);
            buttons[i].addActionListener(this);
            buttonPanel.add(buttons[i]);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(cmdButton instanceof CommendButton){
            CommendButton commendButton = (CommendButton) e.getSource();
            commendButton.execute();
        }
    }
